...
UP_KEY_CODE = 'Down' if platform == "darwin" else 111
DOWN_KEY_CODE = 'Up' if platform == "darwin" else 116
LEFT_KEY_CODE = 'Left' if platform == "darwin" else 113
RIGHT_KEY_CODE = 'Right' if platform == "darwin" else 114
...
def scan_wifi():
    networks = Cell.all('wlan0') # You might need to change 'wlan0' depending on your system
    return networks
...
class tkinterApp(tk.Tk): 
    ...
    def __init__(self, *args, **kwargs):  
        ...
        for F in (SplashFrame, StartPage, NowPlayingFrame, SearchFrame): 
    ...
    def set_splash_image(self, image_path):
        splash_frame = self.frames[SplashFrame]
        splash_frame.update_image(image_path)
...
class SplashFrame(tk.Frame):
    # Show a splash screen while we wait for the app to start
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        self.configure(bg=SPOT_BLACK)

        # Image
        original_image = Image.open("splash_screen_1.png")
        
        # Force resize to 320x240
        if platform == 'darwin':
            original_image = original_image.resize((320, 240), Image.ANTIALIAS)

        self.photo_image = ImageTk.PhotoImage(original_image)  # Keep a reference

        self.image_label = tk.Label(self, image=self.photo_image)
        self.image_label.pack(fill=tk.BOTH, expand=True)

    def update_image(self, image_path="splash_screen_1.png"):
...
def onKeyPress(event):
    c = event.keysym if platform == 'darwin' else event.keycode
    if (c == UP_KEY_CODE):
        onUpPressed()
...
# Create the queue to hold progress messages
progress_queue = queue.Queue()

# Show Splash Screen
app = tkinterApp()
app.set_splash_image("splash_screen_1.png")
...
# Refresh Data
refresh_thread = threading.Thread(target=start_refresh, args=(progress_queue,))
app.after(1000, refresh_thread.start)

def app_main_loop():
    global app, page, loop_count, last_interaction, screen_on, isStartingUp
    if (isStartingUp):
        try:
            ...
            if message:
                if (message == "done"):
                    ...
                    app.set_splash_image(message)
        except queue.Empty:
            pass

        app.after(100, app_main_loop)
        return
