...
def start_refresh(queue):
    spotify_manager.refresh_data(queue)
...
class WifiPage(MenuPage):
    def __init__(self, previous_page):
        super().__init__("WiFi Network", previous_page, has_sub_page=True)
        self.is_title = False
    ...
    def get_available_networks(self):
        if platform == "darwin":
            try:
                ...
                networks = [line.split()[0] for line in result.splitlines()[1:]]  # skipping the header
                networks.sort()
                return networks
            except:
                print("Error fetching WiFi networks.")
                return []
        else:
            try:
                result = subprocess.check_output(["sudo", "iwlist", "wlan0", "scan"])
                result = result.decode("utf-8")  # Convert bytes to string
                networks = [line.split('ESSID:"')[1].split('"')[0] for line in result.split('\n') if "ESSID:" in line]
                networks.sort()
                return networks
            except:
                print("Error fetching WiFi networks.")
    ...
    def page_at(self, index):
        network_name = self.networks[index]
        return NetworkDetailPage(self, network_name)

    def connect_to_network(self, network_name):
        # Implement this method to connect to a selected WiFi network.
...
class NetworkDetailPage(MenuPage):
    def __init__(self, previous_page, network_name):
        super().__init__(network_name, previous_page, has_sub_page=False)  
    ...
    def total_size(self):
        return 0  # Since we're not listing anything for now.

    def page_at(self, index):
        # Nothing to return for now since it's a blank page.
        pass

    def render(self):
        # For now, let's just inherit the base rendering of MenuPage.
        return super().render()
...
class RootPage(MenuPage):
    def __init__(self, previous_page):
        super().__init__("Paul's Spotify", previous_page, has_sub_page=True)
        self.pages = [
            ArtistsPage(self),
            AlbumsPage(self),
